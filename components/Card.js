...
const Card = ({ data }) => {
  ...
  function createBadge(types) {
    ...
    return (
      <HStack m={3} space={1}>
        {types.map((ele, i) => {
          return <Box px={2} py={1} bg={"success.600"} borderRadius={200} key={i}>
  ...
  function cardFooter(gained, launched, days) {
    return (
      <HStack m={3}>
        <Box flex={1}>
          {(launched == 1 || launched == -1) && <VStack justifyContent={"center"} alignItems="center">
            <Heading fontSize={"sm"}>Total Raised </Heading>
            <Heading fontSize={"lg"}>{gained + "%"}</Heading>
          </VStack>}
          {launched == 0 && <VStack justifyContent={"center"} alignItems="center">
        ...
        <Box flex={1}>
          ...
          {launched == 1 &&  <VStack justifyContent={"center"} alignItems="center">
